 ...
 */
public class CallSend extends CardSend {

    public CallSend(IFrame frame) {
        super(frame);
    }
    ...
    protected CardInfo createCardInfo(Card card, User user, String comment) {
        ...
        if (card instanceof Call) {
            Call call = (Call) card;
            ci.setDescription(MessageProvider.getMessage(CallSend.class, "Call.comment",
                    (user.getLanguage() != null) ? new Locale(user.getLanguage()) : UserSessionProvider.getUserSession().getLocale())
                    + " " + call.getNumber() + " (" + comment + ")");
        }
        return ci;
    }
}
